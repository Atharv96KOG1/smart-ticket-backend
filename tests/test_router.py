...
def test_route_ticket_escalation_only_applies_to_primary_issue(monkeypatch):
    ...
    result = route_ticket("Networking issue, and by the way I've experienced data loss before too.")
...
def test_route_ticket_falls_back_after_exhausting_retries(monkeypatch):
    monkeypatch.setattr(router_module, "call_llm", lambda messages: "still not valid json")
